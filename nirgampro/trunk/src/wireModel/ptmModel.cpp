...
// ptmPara
//////////////////////////////////////////////////////////////////////////
ptmModel* ptmModel::clone(){
	return new ptmModel(*this);
}

bool ptmModel::setFieldByName(string name, double val){
...
double ptmModel::getR(ptmModel *p){
	double r;
...
double ptmModel::getCcTop(ptmModel *p){
	double cc;
...
double ptmModel::getCgTop(ptmModel *p){
	double cg;
...
double ptmModel::getCcLocal(ptmModel *p){
	double cc;
	double k = p->K, t = p->t, h = p->h, s = p->s, w = p->w, l = p->length;
...
double ptmModel::getCgLocal(ptmModel *p){
	double cg;
	double w = p->w, h = p->h, s = p->s, t = p->t, k = p->K, l = p->length;
...
UI ptmModel::getDelayTime(){
	if (layer == PTM_TOP){
		return 0.38*(getCcTop(this)+getCgTop(this))*getR(this)/getCharValue('p');
	}
	else
		return 0.38*(getCcLocal(this)+getCgLocal(this))*getR(this)/getCharValue('p');
