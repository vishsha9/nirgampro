...
void TrafficGenerator::send() {
	...
	wait(gc_simWarm);		// wait for gc_simWarm period
	...
	trafstream >> field >> start_latency;
		...
		if(start_latency > 0)
			wait(start_latency);
		// generate traffic until g_tgNum
		while( g_simCount <= gc_simTg && !trafstream.eof()) {
			// format
			...
			//////////////////////////////////////////////////////////////////////////
			//cout << "TG send,  Tile:" << tileID << "\t" << sc_time_stamp() << endl;
			int num_flits_gen = 0;
			flit flit_out;
			...
			while(num_flits_gen < num_flits) {
				...
				//	flit_out = *create_tail_flit(num_pkts_gen, num_flits_gen);
					write_reg_tail(num_pkts_gen, num_flits_gen, route_info, ctrl, 10, 10);
					flit_out = flit_reg;
				}
				else{
					//flit_out = *create_data_flit(num_pkts_gen, num_flits_gen);
					write_reg_data(num_pkts_gen, num_flits_gen, route_info, ctrl, 10, 10);
					flit_out = flit_reg;
				}
...
void TrafficGenerator::recv() {
	while(true) {
		...
		if(flit_inport.event()) {
			...
			flit_recd.simdata.atimestamp = g_simCount;
