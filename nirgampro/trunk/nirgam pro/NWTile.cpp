...
ctr("Controller", nb_num+1){
	...
	if(app_libname[tileID] == "NULL")
		...
		string name = string(1, tileID+'0') + string(1, '__') + app_libname[tileID];
		if(app_libname[tileID].compare("App_test_recv") == 0)
			ip = new App_test_recv(name.c_str());
		else if(app_libname[tileID].compare("App_test_send") == 0)
