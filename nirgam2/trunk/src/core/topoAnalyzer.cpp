...
void showError(ostream & out, int line, string msg){
	...
	out << "> Exit analysis. Plz check the Topology Definition File" << endl;
}
...
AdjList* analyze(string filename, ostream & msg){
	...
	showMsg(msg, "Topology Analyzer for Nirgam Pro, Version " + neta_version);
	showMsg(msg, "");

	input.open(filename.c_str());
	if(!input){
		showError(msg, "Can't find Topology Definition File " + filename);
		return NULL;
	}
	...
	if(fileversion.compare(neta_version)!=0){
		showWarning(msg, lCount, "Topology Analyzer version is not match the input file");
	}
		...
		while(stream >> word){
			...
			if (st.top() == MAIN)
			{
				...
				// WIRE
					...
					if (wireModelForName(type, wp) == false){
				...
				// WIREEND
					...
					if(wpname.size() != 0){
						wpm.insert(pair<string, baseWireModel *>(wpname, wp));
						st.pop();
					}
					else{
				...
				if(strcasecmp( word.c_str(), "CONNECTIONEND") == 0){
					...
					map<string, baseWireModel*>::iterator iter = wpm.find(word);
					if(iter == wpm.end()){
